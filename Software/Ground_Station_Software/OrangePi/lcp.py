...
# *
# * @brief operational software and hardware configuration
# *#

import sys
import OPi.GPIO as GPIO #revised: "RPi.GPIO" to "OPi.GPIO", imports compatible GPIO library for OrangePi One board model
import time
import spidev
import random
import math

try: #revised: (included), structure enhancement
    GPIO.cleanup() #revised: (included), ensures the GPIO has been cleaned prior to a new setup
    ...
    print("##          #######     ##")
    print("##          #######     ##")
    print ("\nLCP script has started.\nType in:\nlcp.diconnect() to disconnect\nor CTRL+C to exit.\n\n") #revised: (included), to instruct how to properly exit the program

    spi_ch = 0

    # Enable SPI
    spi = spidev.SpiDev() #revised: "(0, spi_ch)" to "()", spi setup call
    spi.open(0, spi_ch) #revised: (included) opens spi channel 0
    spi.max_speed_hz = 1000000
    spi.mode         = 0b00 #revised: "0b01" to "0b00" for [CPOL=0|CPHA=0]

    #revised: (included), GPIO single setup as enhancement
    sx1276_DIO0 = 11
    sx1276_DIO2 = 15
    ...
    def radio_reg_cfg():
      ...
      if radio_read(0x42) == 0x12:
          print("SX1276 Configuration OK") #revised: "Radio" to "SX1276"
      else:
          ...
          while True:
              try:
                  ...
                  if ret_val == 1:
                      ...
                      print("******************************************************************")
              except KeyboardInterrupt: #revised: (included), enables the user to stop the loop
                  print("\nEnter new command:")
                  return
      return
      ...
      if frame_type == 1:
          ...
          if (pkt_data[15] & 0x05) == 0x01 :
              ...
              if pkt_data[17] == 0xa3:
                  ...
                  var2 = p*dig_P8/32768
                  pres_absolut_read = (p + (var1+var2+dig_P7)/16)/1000 #Pascal
                  pres_in_mmhg = 0.00750062*pres_absolut_read
                  
                  #CONVERSÕES
                  ntc0 = (2904/(math.log(((int(hex(ntc0_raw),16))*3.22*10**-3)/((3.3-0.7)-((int(hex(ntc0_raw),16))*3.22*10**-3)))+2904/298))-273 #NTC0
                  ntc1 = (2904/(math.log(((int(hex(ntc1_raw),16))*3.22*10**-3)/((3.3-0.7)-((int(hex(ntc1_raw),16))*3.22*10**-3)))+2904/298))-273 #NTC1
                  ldr0 = 10**(-1.4*math.log10(((int(hex(ldr0_raw),16))*32.2)/(3.3-(int(hex(ldr0_raw),16)*3.22*10**-3)))+6.65) #LDR0
                  ldr1 = 10**(-1.4*math.log10(((int(hex(ldr1_raw),16))*32.2)/(3.3-(int(hex(ldr1_raw),16)*3.22*10**-3)))+6.65) #LDR1
                  ldr2 = 10**(-1.4*math.log10(((int(hex(ldr2_raw),16))*32.2)/(3.3-(int(hex(ldr2_raw),16)*3.22*10**-3)))+6.65) #LDR2
                  ldr3 = 10**(-1.4*math.log10(((int(hex(ldr3_raw),16))*32.2)/(3.3-(int(hex(ldr3_raw),16)*3.22*10**-3)))+6.65) #LDR3
                  ldr4 = 10**(-1.4*math.log10(((int(hex(ldr4_raw),16))*32.2)/(3.3-(int(hex(ldr4_raw),16)*3.22*10**-3)))+6.65) #LDR4
                  ldr5 = 10**(-1.4*math.log10(((int(hex(ldr5_raw),16))*32.2)/(3.3-(int(hex(ldr5_raw),16)*3.22*10**-3)))+6.65) #LDR5
                  vcc = (6.44*int(hex(vcc_raw), 16))/1000    #tensão
                  amp = int(hex(amp_raw), 16)/10   #corrente

                  print("SENSORS READING REPLY:")
                  print("NTC0:            ", "%.2f" % ntc0,"     C")
                  print("NTC1:            ", "%.2f" % ntc1,"     C")
                  print("LDR0:            ", "%.2f" % ldr0,"      lux")
                  print("LDR1:            ", "%.2f" % ldr1,"      lux")
                  print("LDR2:            ", "%.2f" % ldr2,"      lux")
                  print("LDR3:            ", "%.2f" % ldr3,"      lux")
                  print("LDR4:            ", "%.2f" % ldr4,"      lux")
                  print("LDR5:            ", "%.2f" % ldr5,"      lux")
                  print("VOLTAGE:         ", "%.2f" % vcc,"      V")
                  print("CURRENT:         ", "%.2f" % amp,"      mA")
                  print("BMP280 Temp:    ", "%.2f" % temp_absolut_read,"     C")
                  print("BMP280 Pres:    ", "%.2f" % pres_absolut_read,"    kPa")
                  print("BMP280 Pres:    ", "%.2f" % pres_in_mmhg, "      mmHg")
    ...
    radio_reg_cfg() #revised: (included), sets up the radio
    radio_rssi_floor() #revised: (included), measures the RSSI level on startup

#revised: (included), function and interruption to exit the program properly
